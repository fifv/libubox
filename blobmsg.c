...
static const int blob_type[__BLOBMSG_TYPE_LAST] = {
    [BLOBMSG_TYPE_INT8] = BLOB_ATTR_INT8,     [BLOBMSG_TYPE_INT16] = BLOB_ATTR_INT16,   [BLOBMSG_TYPE_INT32] = BLOB_ATTR_INT32,
    [BLOBMSG_TYPE_INT64] = BLOB_ATTR_INT64,   [BLOBMSG_TYPE_DOUBLE] = BLOB_ATTR_DOUBLE, [BLOBMSG_TYPE_STRING] = BLOB_ATTR_STRING,
...
int blobmsg_parse_array(
    const struct blobmsg_policy *policy, int policy_len, struct blob_attr **tb, void *data, unsigned int len
) {
    ...
    __blob_for_each_attr(attr, data, len) {
        if (policy[i].type != BLOBMSG_TYPE_UNSPEC && blob_id(attr) != policy[i].type) {
            continue;
        }
...
int blobmsg_parse(const struct blobmsg_policy *policy, int policy_len, struct blob_attr **tb, void *data, unsigned int len) {
    ...
    __blob_for_each_attr(attr, data, len) {
        ...
        for (i = 0; i < policy_len; i++) {
            ...
            if (policy[i].type != BLOBMSG_TYPE_UNSPEC && policy[i].type != BLOBMSG_CAST_INT64 &&
                blob_id(attr) != policy[i].type) {
                continue;
            }

            if (policy[i].type == BLOBMSG_CAST_INT64 &&
                (blob_id(attr) != BLOBMSG_TYPE_INT64 && blob_id(attr) != BLOBMSG_TYPE_INT32 &&
                 blob_id(attr) != BLOBMSG_TYPE_INT16 && blob_id(attr) != BLOBMSG_TYPE_INT8)) {
                continue;
            }
...
__attribute__((format(printf, 3, 4))) int blobmsg_printf(struct blob_buf *buf, const char *name, const char *format, ...) {
    va_list ap;
    int ret;
