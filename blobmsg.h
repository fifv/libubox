...
static inline int
blobmsg_parse_attr(const struct blobmsg_policy *policy, int policy_len, struct blob_attr **tb, struct blob_attr *data) {
    return blobmsg_parse(policy, policy_len, tb, blobmsg_data(data), blobmsg_len(data));
}

static inline int
blobmsg_parse_array_attr(const struct blobmsg_policy *policy, int policy_len, struct blob_attr **tb, struct blob_attr *data) {
    return blobmsg_parse_array(policy, policy_len, tb, blobmsg_data(data), blobmsg_len(data));
}

static inline int blobmsg_add_double(struct blob_buf *buf, const char *name, double val) {
    union {
        double d;
        uint64_t u64;
    } v;
...
static inline int blobmsg_add_blob(struct blob_buf *buf, struct blob_attr *attr) {
    return blobmsg_add_field(buf, blobmsg_type(attr), blobmsg_name(attr), blobmsg_data(attr), blobmsg_data_len(attr));
}
...
int blobmsg_printf(struct blob_buf *buf, const char *name, const char *format, ...) __attribute__((format(printf, 3, 4)));

#define blobmsg_for_each_attr(pos, attr, rem)                                                                                    \
    for (rem = attr ? blobmsg_data_len(attr) : 0, pos = (struct blob_attr *)(attr ? blobmsg_data(attr) : NULL);                  \
         rem >= sizeof(struct blob_attr) && (blob_pad_len(pos) <= rem) && (blob_pad_len(pos) >= sizeof(struct blob_attr));       \
