...
static const struct json_handler expr[] = {
    {"eq", handle_expr_eq}, {"regex", handle_expr_regex}, {"has", handle_expr_has},     {"and", handle_expr_and},
    {"or", handle_expr_or}, {"not", handle_expr_not},     {"isdir", handle_expr_isdir},
};
...
static int json_process_expr(struct json_call *call, struct blob_attr *cur) {
    ...
    if (blobmsg_type(cur) != BLOBMSG_TYPE_ARRAY || blobmsg_type(blobmsg_data(cur)) != BLOBMSG_TYPE_STRING) {
        ctx->handle_error(ctx, "Unexpected element type", cur);
        return -1;
    }
...
void json_script_free(struct json_script_ctx *ctx) {
    ...
    avl_remove_all_elements(&ctx->files, f, avl, next) __json_script_file_free(f);
