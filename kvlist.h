...
#define KVLIST_INIT(_name, _get_len) {.avl = AVL_TREE_INIT(_name.avl, avl_strcmp, false, NULL), .get_len = _get_len}
...
#define kvlist_for_each(kv, name, value)                                                                                         \
    for (value = (void *)__avl_list_to_kv((kv)->avl.list_head.next)->data, name = (const char *)__ptr_to_kv(value)->avl.key,     \
        (void)name;                                                                                                              \
