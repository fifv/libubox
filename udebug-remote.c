...
struct udebug_snapshot *udebug_remote_buf_snapshot(struct udebug_remote_buf *rb) {
    ...
    if (data_size > rb->buf.data_size || ptr_size > rb->buf.ring_size) {
        fprintf(
            stderr, "Invalid data size: %x > %x, %x > %x\n", data_size, (int)rb->buf.data_size, ptr_size, (int)rb->buf.ring_size
        );
        goto out;
    }

    s = calloc_a(sizeof(*s), &ptr_buf, ptr_size * sizeof(*ptr_buf), &data_buf, data_size);
    ...
    while (s->n_entries > 0 && u32_sub(s->entries[0].start, data_used) < 0) {
        s->entries++;
        s->n_entries--;
        s->dropped++;
    }
...
bool udebug_snapshot_get_entry(struct udebug_snapshot *s, struct udebug_iter *it, unsigned int entry) {
    ...
    ptr = &s->entries[entry];
    if (ptr->start > s->data_size || ptr->len > s->data_size || ptr->start + ptr->len > s->data_size) {
        goto error;
    }
