...
static inline int shm_open_anon(char *name) {
...
static int __udebug_buf_map(struct udebug_buf *buf, int fd) {
    ...
    pad = page_size;
#endif
    ptr = mmap(NULL, buf->head_size + 2 * buf->data_size + pad, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (ptr == MAP_FAILED) {
        return -1;
    }
    ...
    ptr2 = mmap(ptr, buf->head_size + buf->data_size, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, fd, 0);
    if (ptr2 != ptr) {
        goto err_unmap;
    }

    ptr2 = mmap(
        ptr + buf->head_size + buf->data_size, buf->data_size, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, fd, buf->head_size
    );
...
static int writev_retry(int fd, struct iovec *iov, int iov_len, int sock_fd) {
    ...
    msghdr.msg_iov = iov, msghdr.msg_iovlen = iov_len, msghdr.msg_control = fd_buf;
...
static int recv_retry(int fd, struct iovec *iov, bool wait, int *recv_fd) {
    ...
    msghdr.msg_iov = iov, msghdr.msg_iovlen = 1, msghdr.msg_control = fd_buf;
    msghdr.msg_controllen = sizeof(fd_buf);
...
static void udebug_send_msg(struct udebug *ctx, struct udebug_client_msg *msg, struct blob_attr *meta, int fd) {
    struct iovec iov[2] = {{.iov_base = msg, .iov_len = sizeof(*msg)}, {}};
...
static struct udebug_client_msg *__udebug_poll(struct udebug *ctx, int *fd, bool wait) {
    ...
    while (udebug_recv_msg(ctx, &msg, fd, wait)) {
        ...
        if (ctx->poll_handle >= 0) {
            __atomic_fetch_or(&rb->buf.hdr->notify, 1UL << ctx->poll_handle, __ATOMIC_RELAXED);
        }
...
int udebug_buf_open(struct udebug_buf *buf, int fd, uint32_t ring_size, uint32_t data_size) {
    ...
    if (buf->ring_size != buf->hdr->ring_size || buf->data_size != buf->hdr->data_size) {
...
int udebug_connect(struct udebug *ctx, const char *path) {
    ...
    list_for_each_entry(buf, &ctx->local_rings, list) __udebug_buf_add(ctx, buf);
...
void udebug_free(struct udebug *ctx) {
    ...
    avl_for_each_element_safe(&ctx->remote_rings, rb, node, tmp) udebug_remote_buf_unmap(ctx, rb);
}
