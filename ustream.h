...
int ustream_printf(struct ustream *s, const char *format, ...) __attribute__((format(printf, 2, 3)));
int ustream_vprintf(struct ustream *s, const char *format, va_list arg) __attribute__((format(printf, 2, 0)));
...
static inline bool ustream_read_buf_full(struct ustream *s) {
    ...
    return buf && buf->data == buf->head && buf->tail == buf->end && s->r.buffers == s->r.max_buffers;
}
